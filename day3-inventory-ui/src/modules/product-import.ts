...
import { addProduct, getProductBySku, type AddProductInput } from "./product";
...
function validateHeaders(headers: string[]): void {
  ...
  const supportedColumns = new Set<ProductCsvColumn>([...REQUIRED_COLUMNS, ...OPTIONAL_COLUMNS]);
...
function parseProductRow(
  ...
  const minQuantity = parseNonNegativeInteger(row.min_quantity ?? "0", "min_quantity", rowNumber);
