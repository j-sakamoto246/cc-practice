...
export async function listWarehouses(): Promise<Warehouse[]> {
  const client = getClient();
  const result = await client.execute(
    "SELECT id, name, location FROM warehouses ORDER BY name ASC",
  );
...
export async function getStockStatus(productId: string, warehouseId: string): Promise<StockStatus> {
  const client = getClient();
...
export async function listStockMovements(
  ...
  const result = await client.execute({
    sql: `SELECT m.*, p.sku, p.name AS product_name, w.name AS warehouse_name
  ...
  return result.rows.map((row) => ({
    ...
    product_name: row["product_name"] as string,
    warehouse_name: row["warehouse_name"] as string,
    memo: (row["reference_id"] as string | null) ?? "",
  }));
}
