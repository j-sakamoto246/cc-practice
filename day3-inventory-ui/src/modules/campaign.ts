...
export async function applyCampaign(
  ...
  if (today < campaign.start_date || today > campaign.end_date) {
    throw new Error(`キャンペーン期間外です: ${campaign.start_date} 〜 ${campaign.end_date}`);
  }
...
export async function listCampaigns(): Promise<Campaign[]> {
  const client = getClient();
  const result = await client.execute("SELECT * FROM campaigns ORDER BY start_date DESC");
  return result.rows.map(rowToCampaign);
}

export async function listActiveCampaigns(currentDate?: string): Promise<Campaign[]> {
  const client = getClient();
